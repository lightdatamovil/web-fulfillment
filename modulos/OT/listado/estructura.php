<script>
    const appOTListado = (function() {
        ...
        public.open = function() {
            $(".winapp").hide();
            $("#ContainerOTListado").show();
            globalLlenarSelect.tiendas("filtroTiendas_armadoOT")
            appOTListado.getListado();
        };

        public.limpiarCampos = function() {
            $("#filtroTiendas_armadoOT").val("");
        };

        function renderListado() {
            $("#tbodyListado_armadoOT").empty()
            buffer = ""

            if (!g_data.data || g_data.data.length < 1) {
                globalSinInformacion.tablasVacias("tbodyListado_armadoOT", openModulo, "OTs")
                return
            };
